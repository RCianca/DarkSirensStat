...
Allevents_DS = {'Mc':1*allMc[0:quanti]*(1+allz)[0:quanti],
            ...
            'Phicoal':0.00001*np.ones(len(allMc))[0:quanti],
            'chi1z':0.*np.ones(len(allMc))[0:quanti],
            'chi2z':0.*np.ones(len(allMc))[0:quanti]
            #'chi2z':np.zeros(len(allMc))[0:1]
           }

gwfast.gwfastUtils.save_data(COV_SAVE_PATH+'SNR_more_than_50_100.h5', Allevents_DS)
...
np.save(COV_SAVE_PATH+'Cov_SNR_more_than_50_100',totCov_ET)